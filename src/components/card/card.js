...
const Card = ({movie}) => {
  ...
  return  <>
 {
    ...
    <Link to={`/movie/${movie.id}`} style={{textDecoration:"none",color:"white"}}>
     <div className='card'>
        <img className='card_img' src={`https://image.tmdb.org/t/p/original${movie ? movie.poster_path:""}`} alt='movie' />
        ...
        <div className="card_description">{movie ? movie.overview.slice(0,118)+"..." : ""}
        </div>
      </div>
     </div>
        </Link>  
